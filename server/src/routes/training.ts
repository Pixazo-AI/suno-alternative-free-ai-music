  ...
  try {
    ...
    try {
      ...
      res.status(501).json({
        ...
        hint: 'Use the Gradio UI at the Pixazo server URL to auto-label your dataset, then reload it here.',
      });
    }
  } catch (error) {
  ...
  try {
    ...
    try {
      ...
      res.status(501).json({
        ...
        hint: 'Initialize the model in the Pixazo Gradio UI service configuration section, then return here for training.',
      });
    }
  } catch (error) {
