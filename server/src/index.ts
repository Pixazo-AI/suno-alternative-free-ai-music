...
app.get('/health', (_req, res) => {
  res.json({ status: 'ok', service: 'Pixazo API' });
});
...
app.get('/song/:id', async (req, res) => {
  ...
  try {
    ...
    const description = `🎵 ${song.style} • Create your own AI music free on Pixazo`;
...
<head>
  ...
  <meta property="og:site_name" content="Pixazo">
...
<body>
  <p>Redirecting to <a href="${config.frontendUrl}?song=${song.id}">Pixazo</a>...</p>
</body>
</html>`);
  } catch (error) {
...
app.listen(config.port, '0.0.0.0', () => {
  console.log(`Pixazo Server running on http://localhost:${config.port}`);
  console.log(`Environment: ${config.nodeEnv}`);
  console.log(`Pixazo API: ${config.pixazo.apiUrl}`);
